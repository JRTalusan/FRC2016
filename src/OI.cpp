...
OI::OI() {
    ...
    launcherWheelBackward->WhenPressed(new ActuateLauncherWheelCommand(true));
    launcherWheelForward.reset(new JoystickButton(driverjoystick.get(), 4));
    launcherWheelForward->WhenPressed(new ActuateLauncherWheelCommand(false));
...
std::shared_ptr<JoystickButton> OI::getdartforward() {
   return dartForward;
}
std::shared_ptr<JoystickButton> OI::getdartbackward() {
   return dartBackward;
}
std::shared_ptr<JoystickButton> OI::getlauncherwheelforward() {
   return launcherWheelForward;
}
std::shared_ptr<JoystickButton> OI::getlauncherwheelbackward() {
   return launcherWheelBackward;
}
